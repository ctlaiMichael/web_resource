...
@Component({
  selector: 'app-regular-confirm-result',
  templateUrl: './regular-confirm-result.component.html',
  styleUrls: []
})

export class RegularConfirmResultComponent implements OnInit {
  ...
  private _initEvent() {
    ...
    this._logger.log("_initEvent, nextData:", this.nextData);
 ...
 */
  ...
  private sendConfirmData(reqData) {
    ...
    // 外幣
    if (investCcyType == '2') {
      this._mainService.sendReginForeignConfirm(reqData, {}).then(
        (result) => {
        ...
        (errorObj) => {
          this._logger.log("getSingleForeignConfirm, errorObj:", errorObj);
      ...
      this._mainService.sendReginTwdConfirm(reqData, {}).then(
        (result) => {
   ...
   * @param reqData 請求
   */
  private sendResultData(reqData, option) {
    ...
    // 外幣
    if (investCcyType == '2') {
      this._mainService.sendReginForeignResult(reqData, option).then(
        (result) => {
          this._logger.log("sendReginForeignResult, result:", result);
        ...
        (errorObj) => {
          this._logger.log("sendReginForeignResult, errorObj:", errorObj);
      ...
      this._mainService.sendReginTwdResult(reqData, option).then(
        (result) => {
          this._logger.log("sendReginTwdResult, result:", result);
          this.resultData = result.infoData;
          // 基金標的formate顯示
          let fundCode = this._formateServcie.checkField(this.nextData, 'fundCode');
          let fundName = this._formateServcie.checkField(this.nextData, 'fundName');
        ...
        (errorObj) => {
          this._logger.log("sendReginTwdResult, errorObj:", errorObj);
          let error_type = this._formateServcie.checkField(errorObj, 'resultType');
          if (error_type == 'security' || error_type == 'check') {
            this.securityAction = { method: 'error' };
            this.setSecurityError = errorObj;
  ...
  submitSecurity(bakSecurityObj?) {
    if (!bakSecurityObj) {
      ...
      // 組request及安控
      let termData = this._formateServcie.checkField(this.nextData, 'termData');
      this.resultReq = {
        ...
        profitAccount: this._formateServcie.checkField(this.nextData, 'profitAccountID'),
        investMoney: this._formateServcie.checkField(this.nextData, 'investMoney'),
        investDate: this._formateServcie.checkField(this.nextData, 'investDate'),
      ...
      if (fundCcy != 'TWD' && fundCcy != 'NTD') {
        securityReq = this._mainService.modifyReginForeign(this.resultReq);
      } else {
        securityReq = this._mainService.modifyReginTwd(this.resultReq);
      }
