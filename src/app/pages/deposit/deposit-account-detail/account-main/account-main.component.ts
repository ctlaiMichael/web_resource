...
export class AccountMainComponent implements OnInit {
    ...
    ngOnInit() {
        this.route.queryParams.subscribe(params => {
            let account_id = this._formateServcie.checkField(params, 'accountId');
            if (account_id != '') {
                this.defaultAcct.account = params['accountId'];
                this.defaultAcct.currencyCode = params['currencyCode'];
            } else {
    ...
    onChangePage(pageType?: string, pageData?: any) {
        ...
        // 如果帳號查詢錯誤(失敗),顯示白箱不動作
        if (this.hasAcct == false && this.errorMsg != '') {
            this._handleError.handleError({
                title: 'ERROR.TITLE',
    ...
    onPageBackEvent(e) {
        ...
        if (page === 'list-page' && pageType == 'back') {
            ...
            this.totalData['totalCramtCount'] = this._formateServcie.checkObjectList(infoData, 'totalData.totalCrCount');
            this.totalData['totalCramt'] = this._formateServcie.checkObjectList(infoData, 'totalData.totalCrAmt');
            this.totalData['totalDramtCount'] = this._formateServcie.checkObjectList(infoData, 'totalData.totalDrCount');
            this.totalData['totalDramt'] = this._formateServcie.checkObjectList(infoData, 'totalData.totalDrAmt');
            this.showTotal = true; // 顯示總數相關
            // 設定頁面資料，若為第一頁
            if (this.pageCounter == 1) {
                // this._logger.log("onPageBackEvent back, pageCounter == 1", tmp_data);
                ...
                // 補充說明
                let other_info = this._formateServcie.checkObjectList(tmp_data, 'infoData.warningMsg', 'string');
                // this._logger.log("onPageBackEvent back, pageCounter == 1", tmp_data, other_info);
                if (other_info != '') {
                    this.noteData.content_list.push(other_info);
                }
                if (this.nowPageType == 'custom') {
