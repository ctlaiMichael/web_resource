    ...
    getData(reqData: object, option?: object): Promise<any> {
        ...
        return this.send(sendObj, option).then(
            (resObj: ApiResponseOption) => {
                ...
                // 日期處理
                let dateRange = this._formateService.checkObjectList(output.data, 'intDateRange');
                output.data['start'] = this._formateService.checkObjectList(dateRange, 'start');
                output.data['end'] = this._formateService.checkObjectList(dateRange, 'end');
