    ...
    check(str: string, symbol_type: string) {
        ...
        switch (symbol_type) {
            case 'base_symbol':
                ...
                break;
            case 'twd_note':
                const check_list = this.getSymbol('twd_note');
                let tmp_check = StringCheckUtil.checkStr(str, check_list.data, true);
                if (tmp_check) {
                    check_symbol.status = true;
                    check_symbol.msg = '';
                } else {
                    check_symbol.status = false;
                    check_symbol.msg = check_list.msg;
                }
                break;
            default:
                break;
    ...
    getSymbol(symbol_type: string) {
        ...
        switch (symbol_type) {
            case 'base':
                ...
                break;
            case 'twd_note':
                output.msg = 'TWD_TRANSFER.EDIT.SETNOT_SYMBOL';
                output.data = [
                    '~', '=', '!', '"', '%', '&', '*', '<', '>', ';'
                    , '@', '#', '$', '[', ']', '\\', '_', '^', '`', '|'
                    , '-', ':'
