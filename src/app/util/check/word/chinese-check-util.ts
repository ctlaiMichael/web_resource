...
export const ChineseCheckUtil = {
    ...
    checkChinese(str: string, check_type?: any) {
        ...
        if (typeof check_type === 'object') {
            ...
            check_space = (check_type.hasOwnProperty('space') && check_type['space']) ? true : false;
        ...
        for (i = 0; i < str.length; i++) {
            ...
            // 允許空白
            let reg1 = /[\s]/g;
            let is_space = reg1.test(str);
            if (is_space) {
                if (!tmp_check && check_space) {
                    tmp_check = true;
                } else if (!check_space) {
                    if (tmp_check) {
                        data.msg = error_msg['space'];
                    }
                    tmp_check = false;
                }
