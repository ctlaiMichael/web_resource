...
export const AmountUtil = {
    ...
    getCurrency(currency: any): object {
        ...
        if (CommonUtil.checkIsTwd(show_currency)) {
            show_currency = 'NT$';
        }
        let engTxt = /^[A-Za-z]+$/;
        if (!engTxt.test(currency)) {  // 中文不顯示
            show_currency = '';
        }
        return {
    ...
    amount(value: any, arg?: any): any {
        ...
        if (typeof value === 'string') {
            ...
            replace_value = replace_value.toString();
        ...
        if (!set_decimal_length) {
            // decimal_length(以傳進來的為主)
            let tmp_num = replace_value.split('.');
            if (!!tmp_num[1]) {
                let data_len = tmp_num[1].length;
                if (data_len > decimal_length) {
                    decimal_length = data_len;
                }
            }
        }
        // logger.log('make formate', Reg_Formate.test(value), value, final_value);
        // == formate轉換 == //
        if (!final_value) {
     ...
     * @param currency 
     */
    currencyOnly(value: any, currency?: any): any {
        let data = this.amount(value, currency);
        let ccy_str = '';
        if (typeof currency !== 'undefined') {
            const currency_data = this.getCurrency(currency);
