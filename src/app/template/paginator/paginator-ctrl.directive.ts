...
export class PaginatorCtrlDirective implements OnDestroy {
  ...
  private doScroll(mainBox) {
    // this._logger.log("doScroll, nowPage/totalPages:", this.nowPage, this.totalPages);
    if (!mainBox) {
      return false;
    }
    let output = {
    ...
    if (output.distance_percent > this.checkPageDistance
      ...
      this._logger.step('Scroll', 'pageCounter (nowPage < totalPages)', this.nowPage);
      this.onScrollEvent();
    } else if (this.nowPage >= this.totalPages) {
      // this._logger.step('Scroll', 'pageCounter is end, nowPage:', this.nowPage);
