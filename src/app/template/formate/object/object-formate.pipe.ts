...
export class ObjectFieldFormatePipe implements PipeTransform {
    ...
    transform(value: any, args: any): any {
        ...
        if (args instanceof Array) {
            ...
            if (typeof args[3] !== 'undefined' && args[3] == 'zero_ok') {
                zero_type = true;
            }
        }
        ...
        if (output != empty_str) {
            switch (other_formate) {
                ...
                case 'currencyMoney':
                    ...
                    break;
                case 'currency':
                    let ccy_obj = AmountUtil.getCurrency(output);
                    output = ccy_obj['show_currency'];
                    break;
                case 'htDate':
                    output = DateUtil.transDate(output, other_formate_set);
                    break;
                case 'accountFormate':
                    output = AccountMaskUtil.accountNoFormate(output);
                    break;
                case 'htFinancial':
                    output = NumberUtil.toFinancial(output, other_formate_set, undefined, zero_type);
                    break;
                default:
                    // no do
                    break;
