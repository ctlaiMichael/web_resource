...
export class BookmarkComponent implements OnInit, OnChanges {
    ...
    ngOnChanges() {
        ...
        this._logger.log('Bookmark', 'do change key', this.defaultKey, this.setKey, this.showType);
        if (typeof this.specialShow == 'undefined') {
            this.showTag = true;
        } else {
            this.showTag = this.specialShow;
        }
    ...
    onGoEvent(menu) {
        ...
        let isLock = this._formateService.checkObjectList(menu, 'lock');
        if (isLock != true) {
            isLock = false;
        }
        if (isLock) {
